...
class RealTimeCurrencyConverter:
    ...
    def convert(self, from_currency, to_currency, amount):
        ...
        try:
            amount = round(
                amount * self.currencies[to_currency], 4)
        except KeyError:
...
if __name__ == "__main__":
    ...
    if converter.currencies is None:
        print(
            ...
            "из-за ошибки при получении данных."
        )
    else:
        ...
        if exchange_rate is not None:
            print(
                f"Текущий курс: 1 USD = "
                f"{exchange_rate:.4f} RUB"
            )
            ...
            if usd_amount is not None:
                print(
                    f"{rub_amount:.2f} рублей = "
                    f"{usd_amount:.2f} долларов"
                )